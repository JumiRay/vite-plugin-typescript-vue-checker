...
} from 'fork-ts-checker-webpack-plugin/lib/typescript-reporter/extension/TypeScriptEmbeddedExtension';
import fs from 'fs';
import { TypeScriptExtension } from 'fork-ts-checker-webpack-plugin/lib/typescript-reporter/extension/TypeScriptExtension';
import { TypeScriptVueExtensionConfiguration } from 'fork-ts-checker-webpack-plugin/lib/typescript-reporter/extension/vue/TypeScriptVueExtensionConfiguration';
import { VueTemplateCompilerV2 } from 'fork-ts-checker-webpack-plugin/lib/typescript-reporter/extension/vue/types/vue-template-compiler';
import { VueTemplateCompilerV3 } from 'fork-ts-checker-webpack-plugin/lib/typescript-reporter/extension/vue/types/vue__compiler-sfc';
...
function createTypeScriptVueExtension(
    ...
    function getVueEmbeddedSource(fileName: string): TypeScriptEmbeddedSource | undefined {
        ...
        if (isVueTemplateCompilerV2(compiler)) {
            ...
            if (parsed.descriptor && (parsed.descriptor.script || scriptSetup)) {
                const scriptV3 = (parsed.descriptor.script || scriptSetup);
                script = {
